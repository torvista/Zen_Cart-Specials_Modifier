...
// NEW Special
          ...
          $db->Execute('UPDATE ' . TABLE_SPECIALS . ' SET specials_modifier = "' . $modifier . '" WHERE specials_id = ' . (int)$db->insert_ID());
// eof
...
// Specials Modifier
      case 'recalculate':
          specials_modifier_recalculate();
          zen_redirect(zen_href_link(FILENAME_SPECIALS, '', 'NONSSL'));
          break;
// eof
  }
}
        ...
        <div class="row">
          <div class="col-sm-6">
            ...
            <a href="<?php echo zen_href_link(FILENAME_SPECIALS, ($currentPage != 0 ? 'page=' . $currentPage . '&' : '') . (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . 'action=pre_add'); ?>" class="btn btn-primary" role="button" title="<?php echo TEXT_INFO_PRE_ADD_INTRO; ?>"><?php echo TEXT_ADD_SPECIAL_PID; ?></a>
              <?php // Specials Modifier ?>
              <a href="<?php echo zen_href_link(FILENAME_SPECIALS, ($currentPage != 0 ? 'page=' . $currentPage . '&' : '') . (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . 'action=recalculate'); ?>" class="btn btn-warning" role="button"><?= SPECIALS_MODIFIER_RECALCULATE_SPECIALS_PRICES_PERCENT; ?></a>
              <?php // eof ?>
          </div>
            ...
            <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9 configurationColumnLeft">
                <table class="table table-hover">
                    ...
                    <tr class="dataTableHeadingRow">
                        ...
                        <?php // Specials Modifier ?>
                        <th class="dataTableHeadingContent text-center"><?= SPECIALS_MODIFIER_HEADING; ?></th>
                        <?php // eof ?>
                ...
                foreach ($specials as $special) {
                  ...
                  if (isset($sInfo) && is_object($sInfo) && ((int)$special['specials_id'] === (int)$sInfo->specials_id)) {
                    ...
                    <?php // Specials Modifier
                    $tmp = $db->Execute('SELECT specials_modifier FROM ' . TABLE_SPECIALS . ' WHERE products_id=' . (int)$special['products_id'] . ' LIMIT 1');
                    $tmp = $tmp->fields['specials_modifier'];
                    if (substr($tmp, -1) == '%') {
                        // %: show as is
                        $modifier = $tmp;
                    } else {
                        // absolute price
                        // $modifier  = $currencies->format(zen_add_tax($tmp)) . '(' . $tmp . ')';
                        $modifier  = $tmp === '' ? '' : '(' . $tmp . ')';
                    }
                        ?>
                    <td class="dataTableContent text-center"><?= $modifier; ?></td>
                    <?php // eof ?>
